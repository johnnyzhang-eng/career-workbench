...
def _study_specs(focus):
    ...
    specs = [
        (0, "sql", "SQL：读表并独立写第一条查询", 25,
         ...
         "用表、列与查询建立本人数据处理起点"),
        (1, "python", "Python：跟练变量与循环，再尝试岗位筛选", 25,
         "先读懂并跟练基础示例，再保存首次独立尝试、求助方式、空列表与缺字段的运行结果；尚写不出函数也记录卡点",
         "先教学再尝试，区分跟练结果与独立能力"),
        (2, "algorithms", "算法：跟练集合，再尝试查找重复项", 20,
         "先解释列表与集合，再保存首次思路或代码、空输入和无重复样例；复杂度不会算时记录问题",
         "从数据结构和边界开始，不用中等题判定基础"),
