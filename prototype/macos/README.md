...
Web 页的「收起／展开工作台／角落小窗」改变路由；原生窗口跟随路由切成 74×74 图标、360×480 角落面板或最大 1100×760 工作台。图标态上方是原生按钮，用于从别的应用回来时单击展开；角落面板闲置六秒且指针在窗外时透明度降到 0.82。无边框窗口左上角（图标态右上角）有可见的关闭按钮，关闭后从菜单栏 ⌂ 的「打开角落小窗」恢复；菜单栏「退出应用」才会终止窗口进程。窗口保持浮在普通窗口之上，跟随所有 Spaces；菜单栏还可切换整个窗口的鼠标穿透。穿透开启后需用菜单栏恢复点击，不能点窗口本身恢复。
