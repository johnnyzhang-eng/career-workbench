...
func _load_route() -> void:
	...
	for arg in OS.get_cmdline_user_args():
		if arg == "--r2-sequence": r2_sequence = true
		if arg == "--r2-video": r2_video = true
	...
	get_window().title = "目标书桌 · R2 骨骼试验"
	print("R2_VARIANT imported-rig")


func _capture(label: String) -> void:
	if not r2_sequence or label != "idle":
	...
	for entry in [
		["sit-start", "SitToType", 0.0],
		["sit-mid", "SitToType", 0.30],
		["sit-end", "SitToType", 0.60],
...
func _capture_video_frames() -> void:
	...
	while elapsed < 3.0 and frame < 110:
		...
		frame += 1
	print("R2_VIDEO_CAPTURE frames=", frame, " elapsed_seconds=", snappedf(elapsed, 0.001),
