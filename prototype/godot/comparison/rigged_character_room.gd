...
# R2 changes only R1's in-progress seated character. The A room is untouched.
...
func _ready() -> void:
	...
	old_pose.queue_free()
	imported_typing = _import_pose("res://comparison/assets/adult_student_rigged.glb", "Rigged seated student")
	...
	assert(player.has_animation("SitToType") and player.has_animation("TypingLoop"))
	assert(skeleton.get_bone_count() == 8)
	player.get_animation("TypingLoop").loop_mode = Animation.LOOP_LINEAR
	player.animation_finished.connect(_on_animation_finished)
	_update_pose()
	if task_state == "in_progress":
		player.play("SitToType")
	print("R2_GODOT_IMPORT skeleton_bones=", skeleton.get_bone_count(),
		" animations=", names,
...
func _on_animation_finished(name: StringName) -> void:
	if name == &"SitToType" and task_state == "in_progress":
		player.play("TypingLoop")
		print("R2_LOOP_STARTED")
