"""Run the R3 character proof in real Godot windows and record evidence."""
...
GLB = PROJECT / "comparison" / "assets" / "adult_student_rigged_r3.glb"
KEYS = (
    "reach-start", "reach-mid", "reach-end", "typing-start",
...
def glb_info():
    ...
    assert len(payload["skins"]) == 1 and animations == ["ReachToType", "TypingLoop"]
    assert len(payload["skins"][0]["joints"]) == 12
    return {
        "file": str(GLB.relative_to(ROOT)), "bytes": len(raw), "sha256": sha(GLB),
        "meshes": len(payload["meshes"]), "skins": 1, "joints": 12,
...
def run():
    ...
    for width, height in ((360, 320), (1280, 720)):
        for phase in ("day", "evening"):
            folder = HERE / "R3" / f"{width}x{height}-{phase}"
            folder.mkdir(parents=True, exist_ok=True)
            command = [
                str(GODOT), "--path", str(PROJECT),
                "res://comparison/rigged_character_experiment_r3.tscn", "--",
                "--route=baseline", f"--size={width}x{height}", f"--phase={phase}",
                f"--out={folder}", "--capture", "--quit", "--r3-sequence",
            ]
            if width == 360 and phase == "day":
                command.append("--r3-video")
            ...
            frame_rows = [json.loads(line.split("R3_FRAME ", 1)[1])
                          for line in output.splitlines() if "R3_FRAME " in line]
            row_by_key = {row["label"]: row for row in frame_rows}
            # Wrist projection is a bounded contact cue, not a fingertip or
            # collision proof. Larger test angles previously left this box.
            for key in ("typing-left-down", "typing-right-down"):
                assert key in row_by_key
                for wrist in row_by_key[key]["wrists_world"].values():
                    assert -1.295 <= wrist[0] <= -0.345 and -1.43 <= wrist[2] <= -1.15
            images = {key: folder / f"{key}-{width}x{height}-{phase}.png" for key in KEYS}
            ok = (
                result.returncode == 0 and "SCRIPT ERROR" not in output
                and "ERROR:" not in output
                and 'R3_GODOT_IMPORT skeleton_bones=12 animations=["ReachToType", "TypingLoop"] meshes=1 camera_fov=45' in output
            ...
            if width == 360 and phase == "day":
                ...
                assert (len(frames_found) >= 70 and "R3_VIDEO_CAPTURE frames=" in output
                        and "R3_LOOP_STARTED" in output)
                movie = HERE / "R3" / "reach-and-type-360x320-day.mp4"
            ...
            print(f"R3 {width}x{height} {phase}: Godot import and keyframes OK", flush=True)
    manifest = {
        "purpose": "Seat-corrected R3 student with stronger typing motion; original A room, camera, light, UI and R2 controls retained",
        ...
        "builder_sha256": sha(HERE / "build_rigged_character_r3.py"),
        "fixed": {"camera_fov_degrees": 45, "room": "original A", "light": "original A", "UI": "original A", "task": "virtual comparison fixture"},
        "records": records,
    }
    (HERE / "R3" / "manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n")
