...
`run_activity_focus_layout.py` 依次运行 L0/L1、360×320/1280×720、白天/夜晚，先采 idle，再通过脚本把同一虚构任务设为 started 采图，共 16 张真实窗口截图；八次进程均退出 0 且日志无 `SCRIPT ERROR`/`ERROR:`。本机 Godot 4.7.stable.official / GL Compatibility、Retina scale=2；小窗 PNG 720×640 原生像素，对照页用 CSS 显示为 360×320 逻辑尺寸。逐轮参数和 SHA-256 见 `manifest.json`，原始 `.log`/`.json` 在 L0、L1 目录。
