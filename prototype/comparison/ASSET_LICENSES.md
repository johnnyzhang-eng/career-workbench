...
| R2 坐姿打字骨骼实验 GLB | [自制 Blender 脚本与可编辑骨骼母版](rigged_character_experiment/README.md) → `../godot/comparison/assets/adult_student_rigged.glb` | 2026-09-25 基于同仓原创 R1 坐姿制作，沿用 MIT；内置几何、显式权重和两段原创动作，无下载模型、动作、贴图或字体；不替换正式 A | GLB 1 skin/8 joints/2 animations 与 Godot 4.7 实际导入、昼夜和两尺寸关键帧及短视频见 [manifest](rigged_character_experiment/manifest.json)；质量局限见 README |
| R3 椅面接点与打字实验 GLB | [自制 Blender 脚本与可编辑骨骼母版](rigged_character_experiment/R3-README.md) → `../godot/comparison/assets/adult_student_rigged_r3.glb` | 2026-09-25 基于同仓原创 R1 坐姿与 R2 骨骼脚本改造，沿用 MIT；未下载模型、动作、贴图或字体，不替换正式 A | GLB 1 skin/12 joints/2 animations，Godot 两尺寸昼夜实机图/视频及局限见 [R3 manifest](rigged_character_experiment/R3/manifest.json)；不是完整站坐动作或定版美术 |
