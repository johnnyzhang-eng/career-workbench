...
`DirectionProfile` 保存本人主线/次线/观察线、硬约束和逐项任务偏好；代理人可写入 proposed 草案，但 `confirmed_by_user=false` 时草案不能成为 agent 找岗条件。方向排序只由页面展示的确定性权重计算，并同时展示主要偏好依据；它是待真实 JD 校准的相对顺序，不是能力、胜任或录用概率。简历经历摘要属于单独的私有证据线索，不自动填答问卷。本人确认后仍需每个候选方向比较真实 JD，方向可多选、可改回 proposed。
