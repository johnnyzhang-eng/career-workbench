...
- 同一虚构 CET6 目标和 360×480 CSS 视口下，原版“开始行动”按钮顶部在 **744.9 px**，首屏外；新布局在 **391.7 px**，底部 **425.7 px**。房间、人物、任务标题、未开始状态和按钮同屏可见。对照见 [原版浏览器首屏](../../prototype/comparison/compact_ui_hierarchy/baseline-360x480-first-screen.png)与[本版浏览器首屏](../../prototype/comparison/compact_ui_hierarchy/improved-360x480-first-screen.png)。
...
- 目标标题特别长时选择框截短；需要通过展开选择框读取全名。无障碍文本仍完整。这次实机对照只有夜间、一项当天任务；白天、多任务与更长任务标题未做同样的截图验收。房间视觉质量、角色风格、长时间常驻的资源占用及跨应用遮挡也不由此布局试验证明。
