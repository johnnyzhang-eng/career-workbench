...
G5 的下一项受控试验是同场景比较未限帧与低帧率常驻模式。Godot 4.7 的 [`Engine.max_fps`](https://docs.godotengine.org/en/4.7/classes/class_engine.html#class-engine-property-max-fps)可限制渲染帧率以降低功耗，但降帧后的角色动作、点击响应、隐藏恢复和系统时间同步都必须实测；任务日期与截止时间仍从现实时间读取，不能从画面帧数累计。
