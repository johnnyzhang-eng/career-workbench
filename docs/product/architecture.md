...
Alpha 以**单人、私有工作区的本地网页工作台**作为实现假设：不同人运行同一套来源适配器和界面，各自保存筛选条件、收藏与投递记录；复用现有 Python 标准库领域逻辑和 SQLite 事件记录，新增本地 HTTP/UI 层。模块先做清楚，再决定打包/安装方式。公开演示只能使用虚构资料。
...
|---|---|---|---|
| `job_sources` | 用户链接/JD、公开职位接口 → 可重复刷新的岗位快照、来源身份、同步状态与原站链接 | 绕过登录、从单次失败宣称“已关闭” | [#9](https://github.com/johnnyzhang-eng/career-workbench/issues/9) |
...
LLM 负责理解自然语言并提出可反驳的候选：JD 要求、经历与能力的映射、下一道练习、分层提示。确定性代码负责 ID、去重、资源筛选、学习进度与能力证据分离、来源与时间、数据保存和权限。首片不实现判题与间隔复测；未来验证端口须单独定义新题、求助与判定规则。学生负责确认岗位、纠正模型、决定投递；能力状态只因有质量的独立证据而更新。
...
以[完整任务链](delivery.md)和[协作契约](collaboration.md)开发，不按模块数量判断完成。2026-09-19 用户把持续找岗与原站投递入口提为第一条切片 [#9](https://github.com/johnnyzhang-eng/career-workbench/issues/9)：先从一个文档明确的公开招聘接口重复刷新，保存最近成功快照和来源状态；两名虚构用户能用各自本地条件筛选，打开职位详情和投递入口，由本人到原站申请，真实回执才登记提交。具体来源覆盖率另行实测，不能把接口返回当成职位可投保证。下一切片 [#8](https://github.com/johnnyzhang-eng/career-workbench/issues/8) 才走外部课程章节推荐和学习进度；两条路径共享岗位引用，学习不阻断投递。每个 PR 要给出用例、未实现边界、隐私检查和实际运行结果。
