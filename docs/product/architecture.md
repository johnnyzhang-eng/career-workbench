...
以[完整任务链](delivery.md)和[协作契约](collaboration.md)开发，不按模块数量判断完成。首条开发切片用**虚构两类岗位、已人工确认的技能要求和外部 SQL 章节元数据**走通：查看要求与出处 → 解释资源推荐 → 选择并打开原站 → 返回记录进度/证据引用 → 保留待验证能力状态，同时仍能打开原岗位模拟链接。模型、自动采集和自建课程不进入首片；它们不阻断手工确认的资源导航。之后再用真实用户的本地私有材料验收。每个 PR 要给出用例、未实现边界、隐私检查和实际运行结果。
