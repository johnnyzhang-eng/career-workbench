...
| Alpha | #9 每人本地 agent 找岗→受控候选导入→本人核验原站与投递；#8 再接岗位要求→技能映射→外部课程章节→本地学习进度与待验证证据；后续按需接经授权的模型建议 | 首位用户能从本人 agent 找到可核查的岗位入口并正确记录实际投递；解释为何选某个章节、调整推荐并找回学习进度；自报完成不升级为掌握；agent 候选质量、真实岗位覆盖和模型接入分开验收；无未经授权的上传或收费 |
