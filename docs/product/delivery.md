...
| Alpha | 从用户链接或公开来源导入岗位→核对 JD 与原站入口→由用户自带模型提出可追溯缺口草稿→比较两类目标→做一题共通 SQL 诊断→跳转现成资源→独立复测的纵向样板 | 首位用户能纠正 JD 和模型判断的错误、说清为什么选这些方向、独立完成新题并找回原岗位；失败与求助记录不被覆盖；无未经授权的云端上传或付费调用 |
...
第一批公开任务宜围绕：① 一个或多个目标方向与真实岗位入口的产品验证；② 岗位要求、共通/专属技能缺口与证据映射；③ 少量原创 SQL 诊断题、外部资源元数据与独立复测；④ 隐私安全的面试反馈录入；⑤ 外部用户可用性测试。每个 Issue 都要有虚构端到端用例，不能把一位用户的私密录音或简历作为 CI fixture。
