...
[3D 房间品质升级实验](3d-quality-upgrade.md)进一步列出 Blender→glTF/Godot 的角色、材质和动画制作流程、官方素材来源与许可边界，以及在原生小窗可检验的下一轮样本；这仍是待验证方案。
...
| 场景只读接线 | [PR #47](https://github.com/johnnyzhang-eng/career-workbench/pull/47) 从本机 `/api/state` 只转发阶段、动作语义与活动状态给无清单按钮的 Godot Web 房间；在真实 WKWebView 中看过计划中→本人开始学习→计划中三个状态。 | 仍是独立嵌入样机，尚未放进当前工作台，也未测长期资源；计划任务单独存在时画面保持待机。 |
