...
状态：2026-09-24 产品提案，隶属 [Issue #12](https://github.com/johnnyzhang-eng/career-workbench/issues/12)。用户已明确选择“每天真正使用的目标执行系统”为首要价值，并要求底座能支持秋招以外的目标，例如 CET6；秋招求职与技能学习是首批知识重点。具体目标模型、自动调整规则和视觉形式仍待评审，现有样机不代表成品质量。

## 先定产品目的

**已确认的产品方向：**用户设置一个真实目标，系统根据期限、可用时间、起点和来源明确的知识材料提出阶段计划与每日行动；用户执行并留下实际结果，系统依据偏差和新信息提出调整，再让用户确认下一版计划。游戏世界让投入、行动与积累可见，也提供长期陪伴。角落小窗是入口之一，产品主体是目标、执行、反馈和调整的闭环。
