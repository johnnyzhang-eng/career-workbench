...
`GoalDailyBridge.status(goal_id)` 不写磁盘，返回 `no_plan`、`sync_pending`、`applied` 或 `conflict` 和逐任务原因。`sync(goal_id)` 安排尚未写入的任务，也将已接受的 `auto_apply` / `undo_auto` 逐版本重放成 `reschedule` 事件。每项 schedule 和每次微调都有由目标 ID、版本和任务 ID 推导的稳定事件 ID。接受计划先在 `GoalStore` 落盘，因此即使某个每日任务写入后进程退出，重启后再次执行 `sync` 会核对已写入前缀并补齐缺项。桥接时对目标库持有写事务，避免另一个连接同时更换其生效计划；日任务的幂等校验处理重复写入。
...
自动微调只允许保留计划项其他内容完全相同；用户接受的内容修改、跨日移动、固定任务、带截止的任务和删除未结束任务依旧显示 `conflict`，等待明确处理，不自行取消或改写。若连续几个版本均有微调，桥接按版本次序补齐审计事件，不直接跳到最终时段。显示顺序按同一目标和本地日期的 `display_order` 排列；其他任务仍按安排时间排列。`reschedule` 不修改完成依据或任务状态，也不能把练习标成掌握、把打开岗位页标成投递。
