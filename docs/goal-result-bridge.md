...
- 当前没有操作系统级桌面身份认证、实际来源文件内容验证、真实 Codex 事件适配器或任意目标的自动计划建议。仅 `actor="user"` 字符串不构成安全认证；本机网页服务还需 Host、Origin 与 CSRF 校验，并只可在本人点击确认时调用桥。
...
- 当前本机 UI 只提供“保留现有任务与时段”的复盘提案；本人接受后需重新读取 `GoalDailyBridge.status`，不能把人工改期提案说成已安排。#33 只覆盖自动安全微调，人工改期投影需另行接入。
